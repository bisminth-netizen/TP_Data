...
def main():
    ...
    for name, fp in INDICATOR_FILES.items():
        ...
        if POLARITY[name] == +1:
            ...
            # ── Unit sanity check ──────────────────────────────────────────
            # The Gaussian parameters (POP_OPTIMUM=8000, POP_SIGMA=12000) are
            # in persons/km².  arr2d MUST be in persons/km² — produced by
            # 01_resample_indicators.py which sums persons/pixel and divides
            # by the 0.25 km² cell area.
            #
            # If arr2d were still in persons/pixel (max ~500), the Gaussian
            # centred at 8000 would assign near-zero scores to every cell,
            # completely collapsing PopDensity's contribution.
            _pop_valid_vals = arr2d[mask & ~np.isnan(arr2d)]
            _pop_max = float(np.nanmax(_pop_valid_vals)) if len(_pop_valid_vals) else 0.0
            _pop_med = float(np.nanmedian(_pop_valid_vals)) if len(_pop_valid_vals) else 0.0
            if _pop_max < 1000:
                raise ValueError(
                    f"PopDensity max={_pop_max:.1f} — values look like persons/pixel "
                    f"(expected persons/km², range roughly 0–50000 for Bangkok).\n"
                    f"  Check that 01_resample_indicators.py ran with "
                    f"POP_INPUT_UNIT='persons_per_pixel' and the sum→km² conversion "
                    f"was applied.  Re-run 01_ before re-running this script."
                )
            print(f"  ✓  PopDensity unit check: max={_pop_max:,.0f}, "
                  f"median={_pop_med:,.0f} persons/km² — Gaussian parameters are consistent.")

            # Apply Gaussian bell directly to raw persons/km² values.
            # Gaussian output is naturally in [0,1]; no further normalisation needed.
            norm2d = np.where(mask,
                              gaussian_transform(
                                  np.where(mask, arr2d, np.nan),
                                  POP_OPTIMUM, POP_SIGMA),
                              np.nan)
