...
Known limitations (report in methodology section)
--------------------------------------------------
1. MAUP at 250 m — nearest-neighbour expansion for coarse-source indicators:
   NTL (500 m native), PM2.5 (500 m native), and Accessibility (500 m derived)
   are expanded to 250 m via nearest-neighbour resampling in 07_maup_sensitivity.py.
   This produces a "blocky" surface with visible 500 m tile boundaries.
   A bilinear/bicubic expansion would be smoother but would inflate spatial
   autocorrelation (the original MAUP bug this script was designed to avoid).
   The blocky artefact is expected and should be reported as a MAUP limitation.

2. PM2.5 spatial variance:
   Bangkok PM2.5 (PCD station-interpolated) has low spatial CV (~2.6%) relative
   to other indicators.  After min-max normalisation the gradient is compressed,
   resulting in near-zero PCA loadings for PM2.5.  Report in limitations.

3. NDVI negative values:
   Water bodies and impervious surfaces produce NDVI < 0.  These are valid
   measurements but widen the normalisation range, reducing relative contrast
   among vegetated urban areas.  See NDVI clipping sensitivity in file 03.

4. Euclidean distance vs. network routing:
   Accessibility scores use straight-line (Euclidean) distance-decay rather
   than actual travel-time via the road/transit network.  Urban network
   tortuosity and transit gaps are not captured.  Correlation with network-
   routed scores is typically r > 0.90 in dense grids, but should be noted
   as a methodological limitation (see 02_accessibility_scoring.py).
