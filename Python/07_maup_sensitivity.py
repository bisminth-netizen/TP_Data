...
#
# NOTE — nearest-neighbour expansion artefact (report as MAUP limitation):
#   NTL (500 m native), PM2.5 (500 m native) and Accessibility (500 m derived)
#   are expanded to 250 m via nearest-neighbour.  This creates a "blocky"
#   output where each 500 m source pixel is tiled into a 2×2 block of identical
#   250 m cells, producing visible 500 m tile boundaries in the 250 m maps.
#   A bilinear expansion would be smoother but would artificially inflate spatial
#   autocorrelation (the MAUP interpolation artefact this script was designed to
#   avoid).  The blocky pattern is the correct methodological choice and should
#   be disclosed in the limitations section of the report.
RESAMPLE_METHOD = {
    "NTL_VIIRS"   : Resampling.nearest,    # 500 m → expand to 250 m (blocky; see note above)
    "NDVI_S2"     : Resampling.average,    # 10 m  → aggregate to 250 m
    "LST_Landsat" : Resampling.average,    # 30 m  → aggregate to 250 m
    "PM25_PCD"    : Resampling.nearest,    # 500 m → expand to 250 m (blocky; see note above)
    "PopDensity"  : Resampling.sum,        # 90 m  → aggregate by sum to 250 m
    "Accessibility": Resampling.nearest,   # 500 m → expand to 250 m (blocky; see note above)
}
...
def normalise_indicator(arr2d, mask2d, name):
    ...
    if POLARITY[name] == +1:
        ...
        # FIX (sync with 03_normalise_and_pca_usi.py):
        # Apply Gaussian directly to raw persons/km² values rather than to
        # min-max-normalised [0,1] values.  POP_OPTIMUM and POP_SIGMA are
        # already in persons/km² — same units as arr2d — so no rescaling is
        # needed.  Gaussian output is naturally in [0,1].
        norm    = np.where(mask2d, gaussian_transform(valid, POP_OPTIMUM, POP_SIGMA), np.nan)
    return norm
...
def local_morans_i(valid_vals, W_sp, n_perms=N_PERMS):
    """
    Local Moran's I with conditional randomisation — consistent with
    04_spatial_analysis.py.

    Permutation scheme: at each iteration the full z vector is permuted and
    I_perm_i = z_i * (W z_perm)_i is computed.  Using the original z_i as the
    left multiplier means z_i is effectively treated as fixed while only the
    neighbourhood values are drawn from the shuffled pool.  This matches the
    standard conditional-permutation LISA approach used in 04_spatial_analysis.py.

    Random seed 42 is fixed to ensure permutation p-values are reproducible and
    ...
    for k in range(n_perms):
        z_perm = rng.permutation(z)         # shuffle neighbourhood values
        lag_p  = W_sp.dot(z_perm)
        I_perm = z * lag_p                  # z_i (original) × lagged z_perm
...
def main():
    ...
    # The 250 m grid has 4× the cells of the 500 m grid.  Weights matrix,
    # indicator arrays, and permutation buffers can easily exceed 4–8 GB RAM.
    # Estimate peak usage and warn (or abort) if available RAM is insufficient.
    _n500  = int(mask_500.sum())
    _n250_est = _n500 * 4                       # approximate valid-cell count at 250 m
    _bytes_per_array = _n250_est * 8             # float64
    _n_arrays        = len(RAW_INPUTS) + 6       # indicator arrays + buffers
    _weight_nnz_est  = _n250_est * 8             # ~8 neighbours per cell (queen)
    _perm_buffer     = _n250_est * 8 * 2         # two permutation arrays
    _peak_mb = (_bytes_per_array * _n_arrays + _weight_nnz_est * 8 + _perm_buffer) / 1e6

    try:
        import psutil
        _avail_mb = psutil.virtual_memory().available / 1e6
        _total_mb = psutil.virtual_memory().total / 1e6
        print(f"\n  RAM check: estimated peak ≈ {_peak_mb:,.0f} MB  |  "
              f"available = {_avail_mb:,.0f} MB / {_total_mb:,.0f} MB total")
        if _peak_mb > _avail_mb * 0.80:
            print(f"\n  ⚠  WARNING: Estimated peak RAM ({_peak_mb:,.0f} MB) exceeds "
                  f"80% of available memory ({_avail_mb:,.0f} MB).")
            print(f"     Risk of OOM kill or excessive swap during 250 m Moran's I "
                  f"permutations ({N_PERMS} perms × {_n250_est:,} cells).")
            print(f"     Options: (a) reduce N_PERMS to 199, "
                  f"(b) close other applications, (c) run on a machine with "
                  f"≥ {int(_peak_mb / 1024) + 2} GB RAM.")
            _ram_ok = input("     Continue anyway? [y/N]: ").strip().lower() == "y"
            if not _ram_ok:
                print("     Aborting 250 m analysis.  "
                      "500 m and 1 km results are already saved.")
                # Skip to 1 km block by returning partial results
                results["250m"] = None
        else:
            print(f"     ✓ RAM check passed — safe to proceed.")
            _ram_ok = True
    except ImportError:
        print(f"\n  RAM check skipped (psutil not installed — "
              f"run: pip install psutil).\n"
              f"  Estimated peak usage ≈ {_peak_mb:,.0f} MB — "
              f"ensure ≥ {int(_peak_mb / 1024) + 2} GB RAM is free before continuing.")
        _ram_ok = True   # proceed optimistically

    if not _ram_ok:
        print("\n[2/3] Skipping 250 m analysis — insufficient RAM (user aborted).")
        results["250m"] = None
    else:
    ...
    for scale in ["250m", "500m", "1km"]:
        if scale not in results or results[scale] is None:
            A(f"  {scale:>6s}  {'(skipped — insufficient RAM)':>50s}")
            continue
        r = results[scale]
    ...
    if (results.get("250m") is not None and
            results.get("500m") is not None and
            results.get("1km") is not None):
