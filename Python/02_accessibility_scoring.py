...
# Healthcare shapefile updated to newer dataset (253 facilities vs 251 in old file).
# The new file is in EPSG:4326 and will be reprojected to UTM47N by
# reproject_points_to_utm() — no extra steps needed.
# Legacy files deleted (no longer needed):
#   BKK_Accessibility_Healthcare.shp          — 251 pts, UTM47N (old)
#   BKK_Accessibility_Healthcare_OSM_backup.shp — 143 pts, UTM47N (OSM backup)
SHP_HEALTHCARE = os.path.join(BASE_DIR,
                               "bangkok_health_facilities_shp",
                               "bangkok_health_facilities.shp")
...
def read_shp_points(shp_path: str) -> np.ndarray:
    ...
    with open(shp_path, "rb") as f:
        ...
        while True:
            ...
            # content_length is stored big-endian per the shapefile spec.
            # Some implementations (or corrupted files) use little-endian.
            # Try big-endian first; fall back to little-endian if the result
            # looks implausible (i.e. content_length * 2 exceeds 100 MB).
            try:
                content_length_be = struct.unpack(">i", header[4:8])[0]
                content_length_le = struct.unpack("<i", header[4:8])[0]
                MAX_RECORD_BYTES  = 100 * 1024 * 1024  # 100 MB sanity cap
                if content_length_be > 0 and content_length_be * 2 <= MAX_RECORD_BYTES:
                    content_length = content_length_be
                elif content_length_le > 0 and content_length_le * 2 <= MAX_RECORD_BYTES:
                    print(f"  ⚠  Endian fallback: big-endian gave implausible "
                          f"record length ({content_length_be}); using little-endian "
                          f"({content_length_le}) for {os.path.basename(shp_path)}")
                    content_length = content_length_le
                else:
                    break  # neither interpretation is valid — stop reading
            except struct.error:
                break
