...
def fig5_gistar():
    ...
    # Minimum raised to 3.0 to ensure ±2.58 significance threshold fits cleanly
    # inside the colorbar range without crowding.  When vlim < 2.58 the ±2.58
    # and ±vlim ticks collapse together, producing overlapping labels.
    vlim_raw = np.nanpercentile(np.abs(data[~np.isnan(data)]), 98)
    vlim     = max(round(vlim_raw * 2) / 2, 3.0)   # at least ±3.0
    ...
    # Ticks at ±1.96 (p<0.05), ±2.58 (p<0.01), and ±vlim.
    # FIX: only include a significance-threshold tick if it is at least
    # MIN_TICK_SEP away from the vlim boundary; otherwise the tick label
    # overlaps with the boundary label and the colorbar becomes unreadable.
    MIN_TICK_SEP = 0.5   # minimum distance (z-score units) between any two ticks
    SIG_TICKS    = [1.96, 2.58]
    tick_candidates = [-vlim, 0.0, vlim]
    for t in SIG_TICKS:
        for signed_t in (-t, t):
            # Include only if strictly inside [-vlim, vlim] with sufficient margin
            if abs(signed_t) < vlim and (vlim - abs(signed_t)) >= MIN_TICK_SEP:
                tick_candidates.append(signed_t)
    cb_ticks = sorted(set(tick_candidates))  # already guaranteed within [-vlim, vlim]
...
def fig6_summary_charts():
    ...
    span = max(abs(load_min), abs(load_max))
    ax_pca.set_xlim(load_min - span * 0.25, load_max + span * 0.25)
...
def fig9_maup_sensitivity():
    ...
    HL_vals = [None, None, None]   # HL + LH outlier cells; typically negligible
    ...
    try:
        ...
        print(f"  [Fig9] Could not parse report: {e} — some values may use fallback.")

    # ── Fallback defaults ──────────────────────────────────────────────────────
    # These are approximate Bangkok reference values from a prior run.
    # They are used ONLY when the corresponding value could not be parsed from
    # maup_sensitivity_report.txt.  Any cell that falls back is flagged below
    # and a visible warning is added to the figure so the chart is never silently
    # mis-labelled with stale / illustrative numbers.
    FALLBACK_DEFAULTS = {
        "I":  [0.85, 0.93, 0.95],
        "HH": [29.0, 30.6, 29.1],
        "LL": [32.0, 32.9, 32.4],
        "NS": [39.0, 36.4, 38.4],
        "HL": [0.0,  0.0,  0.0],   # HL+LH outliers are typically negligible
    }
    _used_fallback = False
    for i in range(3):
        if I_vals[i]  is None: I_vals[i]  = FALLBACK_DEFAULTS["I"][i];  _used_fallback = True
        if HH_vals[i] is None: HH_vals[i] = FALLBACK_DEFAULTS["HH"][i]; _used_fallback = True
        if LL_vals[i] is None: LL_vals[i] = FALLBACK_DEFAULTS["LL"][i]; _used_fallback = True
        if NS_vals[i] is None: NS_vals[i] = FALLBACK_DEFAULTS["NS"][i]; _used_fallback = True
        if HL_vals[i] is None: HL_vals[i] = FALLBACK_DEFAULTS["HL"][i]; _used_fallback = True
    if _used_fallback:
        print("  ⚠  [Fig9] One or more MAUP values could not be parsed from "
              "maup_sensitivity_report.txt.\n"
              "     Fallback (illustrative) values are being used — the figure\n"
              "     will carry a visible warning.  Re-run 07_maup_sensitivity.py\n"
              "     and then re-run this script to use real data.")
    ...
    # If any value came from the hardcoded defaults (parsing failed), overlay a
    # prominent banner on the figure so it is never silently passed off as real.
    if _used_fallback:
        fig.text(
            0.5, 0.50,
            "⚠  ILLUSTRATIVE DATA\nRe-run 07_maup_sensitivity.py\nfor real values",
            ha="center", va="center", fontsize=18, color="#cc0000",
            alpha=0.30, fontweight="bold", rotation=15,
            transform=fig.transFigure,
            bbox=dict(boxstyle="round,pad=0.5", facecolor="#ffeeee",
                      edgecolor="#cc0000", alpha=0.18, linewidth=2),
        )
        fig.text(
            0.5, 0.02,
            "⚠  Warning: one or more MAUP values could not be read from "
            "maup_sensitivity_report.txt — fallback (illustrative) values shown.",
            ha="center", fontsize=7.5, color="#cc0000", style="italic",
            transform=fig.transFigure,
        )
    ...
    print("  Saved → Fig9_MAUP_Sensitivity.png"
          + ("  ⚠ FALLBACK VALUES USED — re-run 07_maup_sensitivity.py" if _used_fallback else ""))
