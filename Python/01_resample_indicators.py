...
# ── PopDensity input unit — SET THIS MANUALLY if you change source data ────────
# "persons_per_pixel"  : source stores persons per pixel (WorldPop constrained,
#                        GHS-POP).  Aggregated by sum then divided by cell area
#                        to produce persons/km².
#                        VERIFIED for current Residential.tif: max ≈ 505 ✓
# "persons_per_km2"    : source already stores a density (some GHSL products,
#                        WorldPop R2025A unconstrained).  Aggregated by average;
#                        no area-division step.
# None                 : auto-detect at runtime based on p99 heuristic
#                        (p99 > 5 000 → assume persons_per_km2, otherwise
#                         persons_per_pixel).  Use only when uncertain.
POP_INPUT_UNIT = "persons_per_pixel"
...
def main():
    ...
    # ~90 m UTM 47N → 500 m.  Strategy depends on POP_INPUT_UNIT (set above).
    TARGET_CELL_AREA_KM2 = (TARGET_RES * TARGET_RES) / 1e6   # 0.25 km²

    # ── Determine effective unit (auto-detect if POP_INPUT_UNIT is None) ────
    with rasterio.open(INPUTS["PopDensity"]) as _src_pop:
        _nd_pop    = _src_pop.nodata if _src_pop.nodata is not None else -99999.0
        _raw_pop   = _src_pop.read(1).astype(np.float64)
        _valid_pop = _raw_pop[(_raw_pop != _nd_pop) & ~np.isnan(_raw_pop)]
    _pop_p99 = float(np.percentile(_valid_pop, 99)) if len(_valid_pop) > 0 else 0.0

    _effective_unit = POP_INPUT_UNIT
    if _effective_unit is None:
        _effective_unit = "persons_per_km2" if _pop_p99 > 5000 else "persons_per_pixel"
        print(f"\n  [Auto-detect] PopDensity p99 = {_pop_p99:.1f} "
              f"→ assumed '{_effective_unit}'")

    # Cross-check: warn if the declared unit seems inconsistent with the data
    if _effective_unit == "persons_per_pixel" and _pop_p99 > 5000:
        print(f"\n  ⚠  UNIT MISMATCH WARNING: POP_INPUT_UNIT='persons_per_pixel' "
              f"but p99={_pop_p99:.1f} > 5000.\n"
              f"     If the source is already in persons/km², set "
              f"POP_INPUT_UNIT='persons_per_km2' to avoid inflating density 4×.\n"
              f"     Proceeding with current setting — verify before using results.")
    elif _effective_unit == "persons_per_km2" and _pop_p99 < 1000:
        print(f"\n  ⚠  UNIT MISMATCH WARNING: POP_INPUT_UNIT='persons_per_km2' "
              f"but p99={_pop_p99:.1f} < 1000.\n"
              f"     If the source stores persons/pixel, set "
              f"POP_INPUT_UNIT='persons_per_pixel'.\n"
              f"     Proceeding with current setting — verify before using results.")
    else:
        print(f"\n  ✓  PopDensity unit check: '{_effective_unit}' "
              f"(p99={_pop_p99:.1f}) — consistent.")

    print(f"[5/5] Population Density (90 m → 500 m, "
          f"method={'sum→persons/km²' if _effective_unit=='persons_per_pixel' else 'average (density)'}) …")

    if _effective_unit == "persons_per_pixel":
        pop = resample_to_grid(INPUTS["PopDensity"], ref_profile, mask,
                               Resampling.sum, nodata_in=-99999.0)
        # sum of persons per 500 m cell ÷ cell area (km²) → persons/km²
        valid_mask = (pop != NODATA_VAL) & (pop >= 0)
        pop = np.where(valid_mask, pop / TARGET_CELL_AREA_KM2, NODATA_VAL)
    else:
        # Source is already persons/km² — average preserves density semantics
        pop = resample_to_grid(INPUTS["PopDensity"], ref_profile, mask,
                               Resampling.average, nodata_in=-99999.0)
